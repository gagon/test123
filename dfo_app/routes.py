...
def integrated_plot_page1():
    ...
    session["dts_data"]=df.values.tolist()
    ...
    timestamps=utils.convert_timestamps(timestamps,"%Y-%m-%dT%H_%M_%S","%Y-%m-%d %H:%M:%S")
    session["dts_timestamps"]=timestamps

    geothermal=utils.read_txt(session["well_geothermal_fullpath"],"geothermal")
    session["geothermal"]=np.array(np.array(geothermal).T).tolist()


    inclination=utils.read_txt(session["well_inclination_fullpath"],"geothermal")
    session["inclination"]=np.array(np.array(inclination).T).tolist()

    # pt=utils.read_pt(session["downhole_data_fullpath"])
    # session["pt"]=np.array(np.array(pt).T).tolist()
